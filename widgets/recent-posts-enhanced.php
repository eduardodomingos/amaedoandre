 ...
 */
class amaedoandre_recent_posts extends WP_Widget {
	function __construct() {
        $widget_ops = array( 'classname' => 'widget_amaedoandre_recent_posts', 'description' => __('Displays most recent posts with featured image and publishing date.', 'amaedoandre') );
        parent::__construct( 'widget_amaedoandre_recent_posts', __('Enhanced Recent Posts','amaedoandre'), $widget_ops );
    }
	function widget($args, $instance) {
		...
		if (!empty($widget_title)) {
			...
			echo $before_title . esc_html__( 'Recent Posts', 'amaedoandre' ) . $after_title;
		}
		?>

			<ul class="amaedoandre-widget-list">

				<?php
					if ( $number_of_posts == 0 ) $number_of_posts = 5;
					$recent_posts = new WP_Query( apply_filters(
					'amaedoandre_recent_posts_args', array(
	...
	function form($instance) {
		...
		<p>
			<label for="<?php echo $this->get_field_id('widget_title'); ?>"><?php  _e('Title', 'amaedoandre'); ?>:
		...
		<p>
			<label for="<?php echo $this->get_field_id('number_of_posts'); ?>"><?php _e('Number of posts to display', 'amaedoandre'); ?>:
			...
			<small>(<?php _e('Defaults to 5 if empty','amaedoandre'); ?>)</small>
		</p>

		<?php
	}
}
register_widget('amaedoandre_recent_posts');
/**
 * Remove the line above and uncomment the lines below to replace the default widget
 */
/*
function amaedoandre_posts_widget_registration() {
  unregister_widget('WP_Widget_Recent_Posts');
  register_widget('amaedoandre_recent_posts');
}
add_action('widgets_init', 'amaedoandre_posts_widget_registration');
*/